...
EVENTS = [
    "page_view", "view_item", "add_to_cart", "begin_checkout", "purchase", "remove_from_cart",
]
...
def build_funnelevents(cfg, rng, sessions: pd.DataFrame, attrs: pd.DataFrame, txns: pd.DataFrame):
    ...
    txn_by_session = dict(zip(txns["session_id"], txns["transaction_id"]))
...
def build_returns(cfg, rng, items: pd.DataFrame) -> pd.DataFrame:
    if len(items) == 0:
        return pd.DataFrame(
            columns=["return_date", "transaction_id", "item_id", "item_quantity",
                     "return_quantity", "return_status"]
        )
