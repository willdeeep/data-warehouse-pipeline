            ...
            if isinstance(node, ast.FunctionDef) and node.name.startswith("test_"):
                ...
                has_marker = any(
                    (isinstance(d, ast.Call) and 
                     ...
                     d.func.attr == REQUIRES_DB_MARK and
                     d.func.value.attr == "mark")
                    or
                    (isinstance(d, ast.Attribute) and
                     isinstance(d.value, ast.Attribute) and
                     d.attr == REQUIRES_DB_MARK and
                     d.value.attr == "mark")
                    for d in node.decorator_list
                )
                if uses_db and not has_marker:
                    failures.append(f"{file_path}:{node.lineno} {node.name} is missing @pytest.mark.requires_db")
    if failures:
        pytest.fail("\n".join(failures))
