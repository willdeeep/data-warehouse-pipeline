...
logging.info(f"CLIENT_ID loaded: {'Yes' if CLIENT_ID else 'No'}")
logging.info(f"CLIENT_SECRET loaded: {'Yes' if CLIENT_SECRET else 'No'}")
if CLIENT_ID:
    logging.info(f"CLIENT_ID length: {len(CLIENT_ID)} characters")
if CLIENT_SECRET:
    logging.info(f"CLIENT_SECRET length: {len(CLIENT_SECRET)} characters")
...
logging.info(f"Using RAW_CSV_PATH: {export_csv}")
...
def extract_raw_data():
        ...
        if not batch:
            ...
            for item in batch:
                ...
                if item.get("price", {}).get("currency") != "GBP":
                    logging.warning(f"Non-GBP price found: {item.get('price')} for item '{item.get('title', '')}' (search term: '{term}')")
    ...
    logging.info(f"Raw data saved to {export_csv}")
