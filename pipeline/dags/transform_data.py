...
def extract_brand_category(search_term, categories):
    """
    Extract brand and category information from eBay search terms.

    This function analyzes search terms to identify known brands and product categories,
    handling common variations and stopwords. It matches against a curated list of
    brands and categories to standardize product classification.

    Args:
        search_term (str): The eBay search term to analyze
        categories (list): List of valid product categories to match against

    Returns:
        tuple: (brand, category) where:
            - brand (str): Identified brand name (title case) or "Unknown"
            - category (str): Identified category (title case) or "Unknown"

    Example:
        >>> extract_brand_category(
            "new nike shoes mens",
            ["shoes",
            "clothing"])
        ("Nike", "Shoes")
...
def extract_gender(search_term):
    """
    Extract gender/demographic information from eBay search terms.

    This function analyzes search terms to determine the target demographic
    (mens, womens, unisex, kids) based on gender-specific keywords. Uses
    a priority system to handle ambiguous cases.

    Args:
        search_term (str): The eBay search term to analyze

    Returns:
        str: Identified gender category ("mens", "womens", "unisex", "kids", or "unknown")

    Example:
        >>> extract_gender("new nike shoes womens")
        "womens"
        >>> extract_gender("kids adidas sneakers")
        "kids"
    """
...
def main():
    """
    Main data transformation function for eBay product data.

    This function performs the complete transformation pipeline:
    1. Loads raw eBay data from CSV
    2. Loads category mappings from BigQuery
    3. Extracts brand, category, and gender from search terms
    4. Applies data cleaning and standardization
    5. Saves transformed data to CSV
    6. Uploads results to BigQuery

    The transformation handles missing values, standardizes product attributes,
    and ensures data quality for downstream analytics.
    """
    ...
    def safe_extract(row):
        """
        Safely extract brand, category, and gender from a DataFrame row.

        This nested function handles missing or invalid data gracefully,
        ensuring the transformation pipeline doesn't fail on edge cases.
        It combines search term analysis with title content to improve
        gender classification accuracy.

        Args:
            row (pd.Series): DataFrame row containing 'search_term' and 'title'

        Returns:
            pd.Series: Series with 'brand', 'category', and 'gender' fields
        """
...
def upload_csv_to_bigquery(csv_path, table_id):
    ...
    try:
        ...
        logger.error("Failed to upload %s to BigQuery: %s", csv_path, e)
