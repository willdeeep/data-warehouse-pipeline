    ...
    def get_operator_args_with_retry_logic():
        ...
        def conditional_args(**context):
            ...
            if task_instance and task_instance.try_number > 1:
                print("🔄 Retry detected (attempt %s), enabling --full-refresh",
...
def send_slack_notification(**kwargs) -> None:
    ...
    if metrics.get('failed_tasks'):
        failure_details.append(
            f"*Failed Tasks:* {len(metrics['failed_tasks'])}")
    ...
    if metrics.get('models_failed', 0) > 0:
        performance_summary.append(
            f"*Models Failed:* {metrics['models_failed']}")
    ...
    message_blocks = [
        {
            ...
            "text": {
                "type": "plain_text",
                "text": "🚨 dbt Pipeline Failure Alert"
            }
        },
        {
            "type": "section",
            "fields": [
                ...
                {
                    "type": "mrkdwn",
                    "text": f"*Failed Task:* {task_instance.task_id}"
                },
                {
                    "type": "mrkdwn",
...
def send_success_notification(**kwargs) -> None:
    ...
    if metrics.get('models_succeeded', 0) > 0:
        success_metrics.append(
            f"*Models Built:* {metrics['models_succeeded']}")
    ...
    model_tasks = [
        task for task in metrics.get(
            'succeeded_tasks', []) if any(
            keyword in task.lower() for keyword in [
                'run', 'build', 'seed'])]
    test_tasks = [task for task in metrics.get('succeeded_tasks', [])
    ...
    if model_tasks:
        success_summary.append(
            f"• {len(model_tasks)} data models built successfully")
    if test_tasks:
        success_summary.append(
            f"• {len(test_tasks)} data quality tests passed")
    ...
    message_blocks = [
        ...
        },
        {
            ...
            "fields": [
                ...
                {
                    "type": "mrkdwn",
                    "text": "*Documentation:* Updated & Published 📚"
                }
            ]
        }
    ]
...
def warehouse_dag_pipeline():
    ...
    def cleanup_on_retry(**context):
        ...
        if dag_run:
            ...
            # operations
            for ti in task_instances:
                if ti.task_id.startswith(
                        'dbt_operations.') and ti.try_number > 1:
                    retry_detected = True
                    print(
                        f"🔄 Retry detected for task: {ti.task_id} (attempt {ti.try_number})")
                    break

        if retry_detected:
            print(
                "🧹 Running dbt with --full-refresh to resolve materialization conflicts...")
            try:
                ...
                result = subprocess.run([
                    ...
                    '--target', 'prod',
                    '--profiles-dir', str(DBT_ROOT_PATH),
                    '--project-dir', str(DBT_ROOT_PATH)
                ], capture_output=True, text=True, cwd=str(DBT_ROOT_PATH))
